...
def parse_commits (logs)
  ...
  logs = logs.map do |log|
    ...
    comments = l.join("\n")
    {
      ...
      :name => name,
      :email => email,
      :date => date != "" ? Time.parse(date) : date,
      :comments => comments
    }
  end
  return logs
...
$last_gh_commit = parse_commits(%x[git log origin/master])[0]
...
task :last_deploy do |t, args|
  require 'pp'
  pp parse_commits(%x[git log origin/master])[0]
end

task :last_deploy_slug do
  puts(parse_commits(%x[git log origin/master])[0].fetch(:date).strftime("%Y%m%d%H%M"))
end
...
task :create_post, [:date, :title, :category, :content] do |t, args|
  ...
  if not File.exists?(post_dir + filename) then
    ...
    sh "code \"#{post_dir}#{filename}\"" if args.content == nil
  else
...
# From https://gist.github.com/jamster/881641
task :git_commit_parser do |t, args|
...
def list_file_changed
  ...
  ld_time = $last_gh_commit.fetch(:date).strftime("%Y%m%d%H%M")
  sh "touch -t \"#{ld_time}\" _last_deploy.txt"
